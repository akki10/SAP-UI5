jQuery.sap.require("Birthdays.util.Formatter");

sap.ui.core.mvc.Controller.extend("Birthdays.view.Master", {
	...
	onInit : function() {
		...
		//-----  prepare query filter by current calendar week number    -------
        var today = new Date();
        var onejan = new Date(today.getFullYear(), 0, 1);
        var weekNumber = Math.ceil((((today - onejan) / 86400000) + onejan.getDay() + 1) / 7);
       
        oList.mBindingInfos.items.path += "?$filter=cust_week%20gt%20" + (weekNumber-2) + "%20and%20cust_week%20lt%20" + (weekNumber+2);
       //-----------------------------------------------------------------------
	...
	loadDetailView : function(){
		this.getRouter().myNavToWithoutHash({ 
			...
			targetViewName : "Birthdays.view.Detail",
			targetViewType : "XML"
		});
	},
	...
	onSearch : function() {
		...
		if (searchString && searchString.length > 0) {
			filters = [ new sap.ui.model.Filter("cust_dateOfBirth", sap.ui.model.FilterOperator.Contains, searchString) ];
		}
		// Update list binding
